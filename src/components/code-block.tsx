...
export default function CodeBlock({
  ...
  return (
    ...
    >
      {fileName && (
        <div className="flex items-center justify-between rounded-t-lg border-b bg-[var(--ds-gray-100)] dark:bg-neutral-800 dark:border-neutral-600 px-2 py-1">
          <div className="flex items-center gap-2">
            <IconComponent className="text-[#a0a0a0]" />
            <span className="text-xs font-medium text-[var(--ds-neutral-900)] dark:text-gray-100">
              {fileName}
            </span>
          </div>
          <div className="flex gap-2">
            <button className="relative cursor-pointer bg-transparent p-2 text-[#a0a0a0] hover:text-[#606060] dark:text-neutral-400 dark:hover:text-neutral-200">
              <Github className="size-4" />
            </button>
            <button
              className="relative cursor-pointer bg-transparent p-2 text-[#a0a0a0] hover:text-[#606060] dark:text-neutral-400 dark:hover:text-neutral-200"
      ...
      <div className="relative flex-1 overflow-auto">
        <div className="sticky top-0 z-10">
          {!fileName && (
            <button
              className={`absolute z-10 cursor-pointer bg-transparent p-2 text-[#a0a0a0] hover:text-[#606060] dark:text-neutral-400 dark:hover:text-neutral-200 ${"right-2 top-1.5"}`}
              onClick={copyToClipboard}
            >
              <div
                className={`transform transition-all duration-200 ${
          ...
          <table className="w-full border-collapse">
            <tbody>
              {content.code.split("\n").map((line, lineIndex) => (
                <tr key={lineIndex}>
                  {showLineNumbers && (
                    <td className="select-none px-4 text-right text-xs text-[var(--ds-gray-700)] dark:text-neutral-400">
                      {lineIndex + 1}
                    </td>
                  )}
                  <td className="w-full">
                    {html[0] && (
                      <code
                        className={`light:[&_span]:!text-[var(--shiki-light)] dark:[&_span]:!text-[var(--shiki-dark)] block h-4 text-xs [&_code]:!bg-transparent [&_pre]:!bg-transparent [&_span]:!bg-transparent ${
